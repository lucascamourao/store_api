...
async def test_usecases_create_should_return_duplicate_error(product_in):
    await product_usecase.create(body=product_in)

    with pytest.raises(DuplicateEntryException) as err:
        await product_usecase.create(body=product_in)

    assert err.value.message == "Product already exists"
